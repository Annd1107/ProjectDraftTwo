...
import { transformEvents, buildEventMap, dateKey, type CalendarEvent } from "../utils/calendar";
...
// ─── OlympiadCalendar ───────────────────────────────────────────────────────
function OlympiadCalendar({ user, cursor, setCursor }: {
  user?: any;
  cursor: Date;
        ...
        >
          ...
          <div className="grid grid-cols-7 gap-1 mb-1">
            {WEEKDAYS.map(d => (
              ...
              <div key={d} className="text-center text-xs text-gray-400 py-1 hidden md:block">{d}</div>
            ))}
          </div>
            ...
            {cells.map(({ d, inMonth }, i) => {
              ...
              return (
                <div
                  ...
                  className={`
                    min-h-[36px] sm:min-h-[52px] rounded-xl p-1 sm:p-1.5 transition-all duration-150 relative
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-0.5 mt-0.5">
                    {evts.slice(0, 4).map((e: any, j: any) => {
              ...
              {selectedEvents.map((e, i) => {
                ...
                return (
                  <div key={i} className="flex items-start gap-3 px-4 py-3 border-b last:border-b-0 border-violet-100 dark:border-violet-900/50 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                    ...
                    <Link
                      to={user ? `/tournament/${e.id}` : "/login"}
                      className={`flex-shrink-0 text-xs px-3 py-1.5 rounded-lg font-medium hover:opacity-90 transition-opacity ${
                        // ✅ check if student is registered for this event
                        user && registrations.some(r => r.olympiad_id === e.id && r.student_id === user.id)
...
export function Home() {
  ...
  // ✅ cursor lives here so it persists when navigating back
  const [cursor, setCursor] = useState(
    () => {
      const saved = sessionStorage.getItem("calendarMonth");
      return saved ? new Date(saved) : new Date(now.getFullYear(), now.getMonth(), 1);
    }
  );

  // ✅ save to sessionStorage whenever cursor changes
  useEffect(() => {
    sessionStorage.setItem("calendarMonth", cursor.toISOString());
  }, [cursor]);
...
<div className="min-h-[36px] sm:min-h-[52px] rounded-xl p-1 sm:p-1.5 ..."></div>
        ...
        <OlympiadCalendar user={user} cursor={cursor} setCursor={setCursor} />
        ...
        </section></div>) : (<div><OlympiadCalendar user={user} cursor={cursor} setCursor={setCursor} /></div>)}
